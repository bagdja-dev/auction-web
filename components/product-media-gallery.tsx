...
export interface ProductMediaGalleryProps {
  ...
  /** Teks alt gambar / dipakai juga sebagai label lightbox — biasanya nama produk. */
  alt: string;
}
...
/**
 * Galeri media produk (foto carousel + zoom + lightbox + video + 3D model)
 * — diekstrak dari `components/templates/default/product-detail-view.tsx`
 * (halaman detail produk publik) supaya bisa dipakai bersama di halaman
 * status Order/Settlement (buyer) dan halaman detail listing (seller,
 * `toko-saya/listing/[product_id]`) — sebelumnya tiap halaman itu cuma
 * tampilkan satu gambar statis (atau tidak sama sekali), bukan galeri
 * lengkap seperti di halaman produk publik. SEMUA state (index slide, zoom,
 * lightbox, swipe) mandiri di komponen ini — cuma butuh `images`/`videoUrl`/
 * `model3dUrl` sebagai props, tidak terikat ke konteks bidding/checkout
 * apa pun.
 */
export function ProductMediaGallery({ images, videoUrl, model3dUrl, alt }: ProductMediaGalleryProps) {
  const imageUrls = images && images.length > 0 ? images : [];
  const slides: Slide[] = [
    ...imageUrls.map((url) => ({ type: 'image' as const, url })),
    ...(videoUrl ? [{ type: 'video' as const, url: videoUrl }] : []),
    ...(model3dUrl ? [{ type: 'model' as const, url: model3dUrl }] : []),
  ];
            ...
            {slides.map((slide, i) => (
              ...
              >
                {slide.type === 'video' ? (
                  ...
                  <>
                    <ModelViewerElement src={slide.url} className="pointer-events-none h-16 w-16 sm:h-20 sm:w-20" cameraControls={false} />
                    <span className="absolute inset-0 flex items-center justify-center bg-black/25 text-white">
                      <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        >
          ...
          {current?.type === 'model' && (
            <div onClick={(e) => e.stopPropagation()}>
              <ModelViewerElement key={current.url} src={current.url} className="aspect-square w-full" cameraControls autoRotate />
            </div>
          )}
          {current?.type === 'image' && (
            <>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={current.url} alt={alt} className="aspect-square w-full object-cover" />
              <div
                aria-hidden
      ...
      {isLightboxOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4" onClick={() => setIsLightboxOpen(false)}>
          ...
          >
            ...
            {current?.type === 'model' && (
              <div onClick={(e) => e.stopPropagation()}>
                <ModelViewerElement key={current.url} src={current.url} className="h-[70vh] w-full max-w-full rounded-lg" cameraControls autoRotate ar />
              </div>
            )}
            {current?.type === 'image' && (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={current.url} alt={alt} className="max-h-[85vh] max-w-full rounded-lg object-contain" />
            )}
